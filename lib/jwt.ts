...
export function generateAccessToken(payload: TokenPayload): string {
  if (!JWT_SECRET || JWT_SECRET === "your-secret-key-change-in-production") {
    throw new Error(
      "JWT_SECRET environment variable is not set. Please set it in Vercel environment variables.",
    );
  }
...
export function generateRefreshToken(payload: TokenPayload): string {
  if (
    ...
    throw new Error(
      "JWT_REFRESH_SECRET environment variable is not set. Please set it in Vercel environment variables.",
    );
  }
  // Type assertion needed due to strict jsonwebtoken types - string values work fine at runtime
...
export function verifyRefreshToken(token: string): TokenPayload | null {
  if (
    ...
    console.error(
      "JWT_REFRESH_SECRET is not set. Token verification will fail.",
    );
    return null;
  }
  try {
...
export function generateSignupToken(payload: {
  phone?: string;
  email?: string;
}): string {
  if (!JWT_SECRET || JWT_SECRET === "your-secret-key-change-in-production") {
  ...
  return jwt.sign({ ...payload, purpose: "SIGNUP" } as object, JWT_SECRET, {
