...
import { FaHistory, FaFilter, FaCalendarAlt, FaUser, FaBuilding, FaChevronLeft, FaChevronRight, FaTimes, FaArrowLeft } from 'react-icons/fa'
...
export default function HallBookingHistoryPage() {
  ...
  const { data: historyResponse, isLoading } = useQuery({
    queryKey: ['hall-booking-history', page, filters],
    queryFn: () => api.get(`/function-hall-bookings/history?${queryParams.toString()}`),
    staleTime: 0,
      ...
      <div className="w-full px-4 lg:px-6 py-4 relative z-10">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">Audit trail of convention hall booking updates and actions</p>
          </div>
        ...
        {history && history.length > 0 ? (
            ...
            <div className="p-4">
              <div className="space-y-4">
                {history.map((entry: any) => {
                  ...
                  const customerName = entry.booking?.customerName || 'Customer'
                  const hallLabel = entry.booking?.hall?.name || '—'
                  ...
                  return (
                    <div key={entry.id} className="relative">
                      ...
                      <div className="absolute left-[6px] top-5 w-4 h-4 rounded-full bg-slate-900 border border-white/10 flex items-center justify-center">
                        <div className={`w-2 h-2 rounded-full ${entry.action === 'CANCELLED' ? 'bg-red-500' : entry.action === 'CREATED' ? 'bg-emerald-500' : 'bg-sky-500'}`} />
                      </div>
                      ...
                      <div className="pl-10">
                        <div className="bg-slate-800/40 border border-white/5 rounded-2xl p-4 hover:bg-slate-800/60 transition-colors">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="mt-2 flex flex-wrap items-center gap-3">
                                <div className="flex items-center gap-1.5 text-sm">
                                  ...
                                  {hasBooking && (
                                    <Link href={`/function-halls/bookings/${entry.bookingId}`} className="text-xs text-sky-400 hover:text-sky-300">
                                      View →
                                    </Link>
                                  )}
                                </div>
                                ...
                                <span className="text-xs text-slate-600">|</span>
                                <div className="flex items-center gap-1.5 text-sm text-slate-300 min-w-0">
                                  <FaBuilding className="w-3 h-3 text-slate-500" />
                          ...
                          {entry.action === 'CREATED' && (() => {
                            ...
                            const paid = getChangeValue(changes, 'advanceAmount') || entry.booking?.advanceAmount
            ...
            {pagination.totalPages > 1 && (
              <div className="flex items-center justify-between px-4 pb-4 border-t border-white/5">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div className="text-xs text-slate-400 px-2">Page {page} of {pagination.totalPages}</div>
                  <button
          ...
          <div className="card text-center py-12">
            <div className="flex flex-col items-center">
              ...
              <p className="text-base font-semibold text-slate-300 mb-1.5">No hall booking history found</p>
              <p className="text-xs text-slate-500">
                {Object.values(filters).some((f) => f) ? 'Try adjusting your filters' : 'No hall booking history yet'}
              </p>
            </div>
          </div>
        )}
