...
export async function GET(request: NextRequest) {
  try {
    ...
    if (reportType === "hotel" || reportType === "all") {
      ...
      const hotelSummaryData = [
        { Metric: "Total Bookings", Value: bookings.length },
        {
          ...
          Value: hotelRevenueSummary._sum.totalAmount || 0,
        },
        {
          Metric: "Collected Amount",
          Value: hotelRevenueSummary._sum.paidAmount || 0,
        },
        {
          Metric: "Pending Amount",
          Value: hotelRevenueSummary._sum.balanceAmount || 0,
        },
    ...
    if (reportType === "convention" || reportType === "all") {
      ...
      if (hallBookings.length > 0) {
        const totalHallRevenue = hallBookings.reduce(
          (sum: number, b: any) => sum + (b.grandTotal || b.totalAmount || 0),
          0,
        );
        const totalHallPaid = hallBookings.reduce(
          (sum: number, b: any) => sum + (b.advanceAmount || 0),
          0,
        );
        const totalHallBalance = hallBookings.reduce(
          (sum: number, b: any) => sum + (b.balanceAmount || 0),
          0,
        );
        const totalElectricity = hallBookings.reduce(
          (sum: number, b: any) => sum + (b.electricityCharges || 0),
          0,
        );
        const totalMaintenance = hallBookings.reduce(
          (sum: number, b: any) => sum + (b.maintenanceCharges || 0),
          0,
        );
        ...
        const conventionSummarySheet = XLSX.utils.json_to_sheet(
          conventionSummaryData,
        );
        XLSX.utils.book_append_sheet(
          workbook,
          conventionSummarySheet,
          "Convention Summary",
        );
      }
    }
    ...
    if (reportType === "all") {
      ...
      const overallSummary = [
        ...
        {
          Category: "Convention Revenue (Buchirajuu)",
          Amount: conventionRevenue,
        },
        ...
        { Category: "---", Amount: "---" },
        {
          ...
          Amount:
            hotelRevenue + conventionRevenue - (totalExpenses + totalSalary),
        },
      ];
