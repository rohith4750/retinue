...
export async function GET(
  ...
  try {
    ...
    if (!booking) {
      return Response.json(errorResponse("Not found", "Booking not found"), {
        status: 404,
      });
    }
...
export async function PUT(
  ...
  try {
    ...
    const result = await prisma.$transaction(
      async (tx: any) => {
        ...
        if (guestName != null && guestName !== currentBooking.guest?.name) {
          ...
          changes.push({
            field: "guest.name",
            oldValue: currentBooking.guest?.name,
            newValue: guestName,
          });
        }
        ...
        if (
          ...
          guestIdProof !== currentBooking.guest?.idProof
        ) {
          guestUpdates.idProof = guestIdProof || null;
          changes.push({
            field: "guest.idProof",
            oldValue: currentBooking.guest?.idProof,
            newValue: guestIdProof,
          });
        }
        if (
          guestIdProofType != null &&
          guestIdProofType !== currentBooking.guest?.idProofType
        ) {
          guestUpdates.idProofType = guestIdProofType;
          changes.push({
            field: "guest.idProofType",
            oldValue: currentBooking.guest?.idProofType,
            newValue: guestIdProofType,
          });
        }
        if (
          guestAddress !== undefined &&
          guestAddress !== currentBooking.guest?.address
        ) {
          guestUpdates.address = guestAddress || null;
          changes.push({
            field: "guest.address",
            oldValue: currentBooking.guest?.address,
            newValue: guestAddress,
          });
        }
        if (
          guestType != null &&
          guestType !== currentBooking.guest?.guestType
        ) {
          guestUpdates.guestType = guestType;
          changes.push({
            field: "guest.guestType",
            oldValue: currentBooking.guest?.guestType,
            newValue: guestType,
          });
        }
        ...
        if (
          numberOfGuests != null &&
          Number(numberOfGuests) !== currentBooking.numberOfGuests
        ) {
          updateData.numberOfGuests = Number(numberOfGuests);
          changes.push({
            field: "numberOfGuests",
            oldValue: currentBooking.numberOfGuests,
            newValue: updateData.numberOfGuests,
          });
        }
        if (
          typeof flexibleCheckout === "boolean" &&
          flexibleCheckout !== currentBooking.flexibleCheckout
        ) {
          updateData.flexibleCheckout = flexibleCheckout;
          changes.push({
            field: "flexibleCheckout",
            oldValue: currentBooking.flexibleCheckout,
        ...
        if (checkIn) {
          ...
          if (
            Math.floor(newCheckIn.getTime() / 1000) !==
            Math.floor(currentCheckIn.getTime() / 1000)
          ) {
        ...
        if (checkOut && status !== "CHECKED_OUT") {
          ...
          if (
            ...
            Math.floor(currentCheckOut.getTime() / 1000)
          ) {
        ...
        // If stay dates changed (checkIn or checkOut), recalculate amount and check conflicts
        if (
          (updateData.checkIn || updateData.checkOut) &&
          status !== "CHECKED_OUT"
        ) {
          const finalCheckIn = updateData.checkIn || currentBooking.checkIn;
          const finalCheckOut = updateData.checkOut || currentBooking.checkOut;
          const room = currentBooking.room;
          const currentRoomId = updateData.roomId || currentBooking.roomId;

          // Check for date conflicts on the room (even if room hasn't changed, dates have)
          ...
          if (conflictCheck.hasConflict) {
            throw new Error(
              `Date conflict: Room ${room?.roomNumber || "selected"} is already booked from ${new Date(conflictCheck.conflictingBooking.checkIn).toLocaleDateString()} to ${new Date(conflictCheck.conflictingBooking.checkOut).toLocaleDateString()}`,
            );
          }

          if (room?.basePrice) {
            const oldDays = Math.ceil(
              (currentBooking.checkOut.getTime() -
                currentBooking.checkIn.getTime()) /
                (1000 * 60 * 60 * 24),
            );
            ...
            if (newDays !== oldDays && newDays > 0) {
              ...
              changes.push({
                ...
                oldValue: currentBooking.totalAmount,
                newValue: newTotal,
                note: `Stay duration changed to ${newDays} day(s)`,
              });
            }
          }
        }
        ...
        const stayCheckOut = updateData.checkOut || currentBooking.checkOut;
        if (
          data.action === "EXTEND_STAY" &&
          stayCheckOut > currentBooking.checkOut
        ) {
          const room = currentBooking.room;
          const oldDays = Math.ceil(
            (currentBooking.checkOut.getTime() -
          ...
          const newDays = Math.ceil(
            (stayCheckOut.getTime() - currentBooking.checkIn.getTime()) /
              (1000 * 60 * 60 * 24),
          );
        ...
        if (
          ...
          changes.push(
            ...
            {
              field: "slotId",
              oldValue: currentBooking.slotId,
              newValue: newSlot.id,
            },
          );
        }
        ...
        if (Object.keys(updateData).length === 0) {
          ...
          return {
            updatedBooking: refreshed ?? currentBooking,
    ...
    const updatedBooking = (
      result as { updatedBooking: any; status?: string; changes: any[] }
    ).updatedBooking;
...
export async function DELETE(
  ...
  try {
    ...
    if (permanent) {
      ...
      return Response.json(
        successResponse(null, "Booking permanently deleted"),
      );
    }
